...
import { getAudioRecordById } from 'apiSdk/audio-records';
import { AudioRecordInterface } from 'interfaces/audio-record';

function AudioRecordViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<AudioRecordInterface>(
    () => (id ? `/audio-records/${id}` : null),
    () =>
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('audio_record', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/audio-records/edit/${id}`} passHref legacyBehavior>
                    <Button
