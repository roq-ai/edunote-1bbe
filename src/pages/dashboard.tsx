...
import { getAudioRecords } from 'apiSdk/audio-records';
...
import { AudioRecordListPage as ShowCaseList } from 'pages/audio-records';

function DashboardPage() {
  const { session } = useSession();

  const audioRecordStat = useSWR<PaginatedInterface<AudioRecordInterface>>(
    () => `/audio-records`,
    () => getAudioRecords({ limit: 1, offset: 0 }),
  );

  const photoStat = useSWR<PaginatedInterface<PhotoInterface>>(
    () => `/photos`,
    () => getPhotos({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Audio Records',
      result: audioRecordStat?.data?.totalCount,
      isLoading: audioRecordStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Photos',
      result: photoStat?.data?.totalCount,
      isLoading: photoStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Topics',
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Audio Records
                </Text>
                <NextLink href={`audio-records`} passHref legacyBehavior>
                  <Button
