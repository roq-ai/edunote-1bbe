...
type ColumnType = ColumnDef<PhotoInterface, unknown>;
...
export function PhotoListPage(props: PhotoListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PhotoInterface>>(
    () => `/photos?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: PhotoInterface) => {
    if (hasAccess('photo', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/photos/view/${row.id}`);
    }
  };
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="photo" />
        </Flex>

        {hasAccess('photo', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/photos/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
