...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getPhotosPublic() {
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'photo');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.photo.count(countOptions as unknown),
  ...
  async function getPhotos() {
    ...
    const limit = parseInt(_limit as string, 10) || 20;
    const offset = parseInt(_offset as string, 10) || 0;
    const response = await prisma.photo
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'photo'),
        take: limit,
        skip: offset,
        ...(order?.length && {
          orderBy: getOrderByOptions(order),
